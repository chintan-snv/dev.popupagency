...
class Post_Types {
	...
	public function __construct() {
		...
		add_filter( 'mylisting/get-preview-card-cache', [ $this, 'dynamic_preview_card_contents' ] );
	}
	...
	*/
	...
	public function dynamic_preview_card_contents( $html ) {
		preg_match( '/<div hidden \#vars>(?<vars>.*)<\/div \#vars>$/', $html, $matches );

		if ( ! empty( $matches['vars'] ) ) {
			$vars = json_decode( $matches['vars'], ARRAY_A );
            if ( json_last_error() === JSON_ERROR_NONE && ! empty( $vars ) && is_array( $vars ) ) {
            	foreach ( $vars as $name => $value ) {
            		if ( $name === 'open-now' ) {
            			$open_now = \MyListing\Src\Work_Hours::parse_short_format( $value )->get_open_now();
						$open_now_markup = sprintf(
							'<div class="lf-head-btn open-status listing-status-%s">
		                        <span>%s</span>
		                    </div>',
		                    $open_now ? 'open' : 'closed',
		                    $open_now ? __( 'Open', 'my-listing' ) : __( 'Closed', 'my-listing' )
						);

						$html = str_replace( '<var #open-now></var>', $open_now_markup, $html );
            		}
            	}
            }
