...
class Promotions {
	...
	public function handle_promotion_request() {
		...
		if ( $process === 'buy-package' ) {
			try {
	            ...
	            return wp_send_json( [
	                'status'  => 'success',
	                'redirect' => add_query_arg( 't', time(), $redirect_url ),
	            ] );
			} catch ( \Exception $e ) {
	            return wp_send_json( [
	                'status'  => 'error',
	...
	public function activate_package( $package_id, $listing_id = false ) {
		...
		do_action( 'mylisting/promotion:start', $listing->ID, $package_id );
	...
	public function listing_feed_args( $args, $data ) {
		if ( empty( array_filter( $data['priority_levels'] ) ) || count( $data['priority_levels'] ) === 4 ) {
