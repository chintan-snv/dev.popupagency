...
<div class="tab-content full-width" v-if="currentSubTab == 'cover-details'">
	...
	--><div class="editor-column col-1-3" :class="single.cover_details.length + single.cover_actions.length >= <?php echo $detail_limit ?> ? 'ml-overlay-disabled' : ''">
		<div class="form-section mb10">
			...
			<div class="btn btn-block mb10" @click.prevent="coverDetails().add( 'Event date', 'job_date', 'date' )">Event date</div>
		...
		<draggable v-model="single.cover_actions" :options="{group: 'cover-actions-list', handle: '.row-head'}">
			<div v-for="action, key in single.cover_actions" class="row-item" :class="action === state.single.active_cover_action ? 'open' : ''">
				...
				<div class="row-edit">
					...
					<div class="form-group full-width">
						<label>Label</label>
						<atwho :data="fieldsByType(['text', 'texteditor', 'wp-editor', 'checkbox', 'radio', 'select', 'multiselect', 'textarea', 'email', 'url', 'number', 'location', 'file', 'date', 'password'])" v-model="action.label" template="input"></atwho>
					...
					<div class="form-group full-width" v-if="typeof action.active_label !== 'undefined'">
						...
						<atwho :data="fieldsByType(['text', 'texteditor', 'wp-editor', 'checkbox', 'radio', 'select', 'multiselect', 'textarea', 'email', 'url', 'number', 'location', 'file', 'date', 'password'])" v-model="action.active_label" template="input"></atwho>
					</div>

					<div class="form-group full-width" v-if="typeof action.link !== 'undefined'">
						<label>Link to</label>
						<atwho :data="fieldsByType(['text', 'texteditor', 'wp-editor', 'checkbox', 'radio', 'select', 'multiselect', 'textarea', 'email', 'url', 'number', 'location', 'file', 'date', 'password'])" v-model="action.link" template="input" placeholder="e.g. `tel:[[work_phone]]`"></atwho>
