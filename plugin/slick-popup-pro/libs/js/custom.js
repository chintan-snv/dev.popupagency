jQuery(document).ready(function($) {
	...
	if( external_unloader.length ) {
		...
		external_unloader.click( function(e) {
			...
			id = jQuery(this).attr('data-formid', '-');			
	...
	// Find all externa_selectors classes from animator
	var external_selectors = {};
	$animators = jQuery('.sppro_popup_animator');
	if($animators.length) {
		$animators.each(function() {
			if (!! jQuery(this).attr('data-external_selectors')) {
				$parentBox = jQuery(this).parent(); 
				popupID = $parentBox.attr('data-numid'); 
				...
				external_selectors[popupID] = selector_parts.join(','); 
			}
	...
	jQuery.each(external_selectors, function(index, value) {
		...
		loaderButton.click(function(e) {
			if( loaderButton.tagName().toUpperCase() == 'A' ) {
				e.preventDefault(); 
			}
		...
		external_loader.click( function(e) {
			...
			if(loaderButton.hasClass('menu-item')) {				
				...
				if(!! jQuery(this).attr('data-formid')) {
					loaderID = jQuery(this).attr('data-formid', '-');				
				}
			}
	...
	if($popupBoxes.length) {
		...
		$popupBoxes.each(function(e) {
			...
			var ef = $animator.attr('data-loadeffect'); 
			var speed = $animator.attr('data-loadspeed'); 
			var bodyscroll = $animator.attr('data-bodyscroll'); 	
			
			var activationmode = $animator.attr('data-activationmode'); 			
			var onscroll_type = $animator.attr('data-onscroll_type'); 
			var onscroll_pixels = $animator.attr('data-onscroll_pixels'); 			
			var onscroll_percentage = $animator.attr('data-onscroll_percentage'); 
			var autopopupdelay = $animator.attr('data-autopopupdelay'); 
			var cookieexpiration = $animator.attr('data-cookieexpiration'); 
			var cookieexpirationdays = $animator.attr('data-cookieexpirationdays'); 
			var cf7formid = $animator.attr('data-cf7formid'); 
			var autoclose = $animator.attr('data-autoclose'); 
			var autoclose_time = $animator.attr('data-autoclose_time'); 
			var redirect = $animator.attr('data-redirect'); 
			var redirect_url = $animator.attr('data-redirect_url'); 
			...
			if(activationmode=='autopopup') {
				...
				if(cookie!="closed" && cookie!="closewithsession" && cookie!="submitted") {
					setTimeout(function () { sppro_loader(id); }, $animator.attr('data-autopopupdelay') * 1000);
				}
			}
	...
	document.addEventListener( 'wpcf7mailsent', function( event ) {				
		// Check if multiple popup by checking '-' sign in Popup ID		
		getSubmittedForm = jQuery('.sppro_popup_box[data-cf7formid='+event.detail.contactFormId+']'); 
		if(getSubmittedForm.length) {
			submitted_popup_id = getSubmittedForm.attr('data-numid'); 
			
			$animator = getSubmittedForm.find('.sppro_popup_animator');	

			autoclose = $animator.attr('data-autoclose'); 
			insights = $animator.attr('data-insights'); 
			autoclose_time = $animator.attr('data-autoclose_time'); 
			redirect = $animator.attr('data-redirect'); 
			redirect_url = $animator.attr('data-redirect_url'); 
			
			sppro_update_insights(submitted_popup_id, 'submitted', insights); 
			
			if(redirect=="true" && redirect_url!='') {
				location.replace(redirect_url); 
			}
			else if(autoclose=="true" && autoclose_time!='') {
				setTimeout(function() {
					sppro_unloader(submitted_popup_id); 
	...
	if( button_loader.length ) {
		button_loader.click(function(e) {
			if('Logout'!= jQuery(this).text()) {
				...
				sideBtnsid = jQuery(this).attr('data-popupid');				
...
function sppro_unloader(id) { 
	...
	var ef = $animator.attr('data-unloadeffect'); 
	var speed = $animator.attr('data-unloadspeed'); 
	var activationmode = $animator.attr('data-activationmode'); 
	var time = $animator.attr('data-cookieexpiration'); 
	var bodyscroll = $animator.attr('data-bodyscroll'); 	
	...
	$popup.addClass(ef + ' animated').css({"-webkit-animation-duration": speed+"s","animation-duration": speed+"s"}).one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
		$popup.addClass('manage').removeClass('animated').removeClass(ef); 		
	});
	...
	//$popup.removeClass('animated').removeClass(ef).addClass('manage'); 	
...
function sppro_loader(id) {  
	...
	// return if popup does not exist
	if(!$popup.length) return; 

	var insights = $animator.attr('data-insights');
	var activationmode = $animator.attr('data-activationmode');
	var ef = $animator.attr('data-loadeffect'); 
	var speed = $animator.attr('data-loadspeed'); 
	var bodyscroll = $animator.attr('data-bodyscroll'); 	
	...
	if(id!='') {
		sppro_update_insights(id, 'opened', insights); 
	}
...
function sppro_set_popup(popupID, ef, speed) {
	...
	if ($popup.length) {		
		...
		$animator = $popup.find('.sppro_popup_animator');		
		form_type = $animator.attr('data-form_type'); 
		loadspeed = $animator.attr('data-loadspeed'); 
		...
		if( formAreaHeight >= formHeight ) {
			...
			var autohidemode = $animator.attr('data-autohidemode'); 			
			var cursorbackground = $animator.attr('data-cursorbackground');			
			var cursorcolor = $animator.attr('data-cursorcolor'); 
			var cursorwidth = $animator.attr('data-cursorwidth'); 
			var cursorborderradius = $animator.attr('data-cursorborderradius'); 			
			var cursorbordercolor = $animator.attr('data-cursorbordercolor'); 
			var cursoropacitymax = $animator.attr('data-cursoropacitymax'); 
			var popupTop = $animator.attr('data-popTop');
		...
		sidebtn = $animator.attr('data-sidebtn'); 		
...
function sppro_createCookie(name, value, days) {
	...
	//currentTime = animator.attr('data-unixtime'); 	
...
jQuery(document).ready(function($) {
	...
	jQuery(".logout_button").click(function() {
        ...
        if(logout) {
        	logoutLink = jQuery(this).attr('data-logoutlink');
